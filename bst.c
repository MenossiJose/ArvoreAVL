...
link rotR (Tree t, link h) {
    ...
    //altera altura dos nos
    h->N = 1 + (getHeight(h->l) > getHeight(h->r) ? getHeight(h->l) : getHeight(h->r));
    x->N = 1 + (getHeight(x->l) > getHeight(x->r) ? getHeight(x->l) : getHeight(x->r));
...
link AVLinsertR (Tree t, link h, int item){
    ...
    int balancear = getHeight(h->l) - getHeight(h->r);
    if (balancear > 1) {
